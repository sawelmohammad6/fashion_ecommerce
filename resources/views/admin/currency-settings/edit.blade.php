...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-white">Edit Currency</h1>
        <p class="text-sm text-gray-400 mt-1">Update currency details</p>
    </div>
...
<div class="max-w-2xl">
    <form action="{{ route('admin.currency-settings.update', $currencySetting) }}" method="POST" class="bg-gray-900/50 border border-gray-800 rounded-xl p-6 space-y-6">
        @csrf @method('PUT')

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Currency Name</label>
                <input type="text" name="currency_name" value="{{ old('currency_name', $currencySetting->currency_name) }}" required
            ...
            <div>
                ...
                <input type="text" name="currency_code" value="{{ old('currency_code', $currencySetting->currency_code) }}" required
                       class="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg px-4 py-2.5 text-sm text-gray-300 placeholder-gray-500 outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition" maxlength="10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="symbol" value="{{ old('symbol', $currencySetting->symbol) }}" required
                       class="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg px-4 py-2.5 text-sm text-gray-300 placeholder-gray-500 outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition" maxlength="10">
            ...
            <div>
                ...
                <input type="number" step="0.0001" min="0.0001" name="exchange_rate" value="{{ old('exchange_rate', $currencySetting->exchange_rate) }}" required
                       class="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg px-4 py-2.5 text-sm text-gray-300 placeholder-gray-500 outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition">
        ...
        <div class="flex items-center gap-6">
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="status" value="1" class="sr-only peer" {{ old('status', $currencySetting->status) ? 'checked' : '' }}>
            ...
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="is_default" value="1" class="sr-only peer" {{ old('is_default', $currencySetting->is_default) ? 'checked' : '' }}>
        ...
        <div class="flex items-center gap-3 pt-2">
            <button type="submit" class="px-6 py-2.5 text-sm font-medium rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition">Update Currency</button>
